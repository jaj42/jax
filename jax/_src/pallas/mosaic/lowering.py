...
def _device_id_to_logical(
  ...
  def jax_fn(device_id_val):
    ...
    elif (kernel_type != dest_kernel_type
          and dest_kernel_type == tpu_core.CoreType.SC_SCALAR_SUBCORE):
      # TODO(rdyro): Mosaic requires resolving the core axis when the target is
      # the scalar subcore, but the source is not. Remove this branch once
      # fixed.
