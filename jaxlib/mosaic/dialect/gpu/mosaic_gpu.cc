...
llvm::LogicalResult MultimemLoadReduceOp::verify() {
  ...
  if (llvm::isa<mlir::IntegerType>(source_element_type)) {
    auto int_type = llvm::cast<mlir::IntegerType>(source_element_type);
    ...
    if ((reduction_type == MultimemLoadReductionType::Min ||
        ...
        !getIsSigned().has_value()) {
      return emitOpError() << "Signedness must be specified for Min, Max, or "
